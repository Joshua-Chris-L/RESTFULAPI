...
# HTTP GET - Read Record. Make a get request
@app.route("/get_random_cafe", methods=["GET"])
def get_random_cafe():
    cafes = db.session.query(Cafe).all()
    random_cafe = random.choice(cafes)
    data = jsonify(id=random_cafe.id,
...
@app.route("/all_route", methods=["GET"])
def all_route():
    cafes = db.session.query(Cafe).all()
    cafe_list = []
    for cafe in cafes:
        cafe_dict = jsonify(id=cafe.id,
                            name=cafe.name,
...
def add():
    new_cafe = Cafe(
        ...
        coffee_price=request.form.get("coffee")
    )
...
@app.route("/delete_cafe/<int:cafe_id>", methods=["PATCH"])
def delete_cafe(cafe_id):
    ...
    if api_key == "TopSecretAPIKey":
        ...
        if cafe:
            ...
            return jsonify(error={"Not Found": "Sorry a cafe that id was not found in database"}), 404
    else:
